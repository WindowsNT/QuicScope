...
#define XML3_VERSION_REVISION_DATE "11-12-2025"
...
std::string Char2Base64(const char *Z,size_t s,bool = true);
void Base64ToChar(const char *Z,size_t s,BXML& out);
std::string Decode(const char* src);
std::string Format(const char* f,...);
std::string Trim(const char* src,int m = 2);
...
class xstring : public std::string
	...
	public:
		...
		xstring() : std::string() {}
		xstring(const char* t) :std::string(t) {}
		xstring(const std::string& t) : std::string(t) {}
		xstring(const xstring& t) : std::string(t) {}
		xstring(xstring&& t) : std::string(std::forward<xstring>(t)) {}
		xstring& operator =(const char* t) { std::string::operator =(t); return *this; }
		xstring& operator =(const std::string& t) { std::string::operator =(t); return *this; }
		xstring& operator =(const xstring& t) { std::string::operator =(t.c_str()); return *this; }
		...
		xstring& operator =(const std::wstring& t)
			{
			operator = (t.c_str());
			return *this;
			}
...
class XMLContent
	...
	public:
		...
		// Sets
		const std::string& SetFormattedValue(const char* fmt,...);
		template <typename T> const std::string& SetFormattedValue(const char* fmt,T ty)
			{
			std::unique_ptr<char> t(new char[10000]);
#ifdef _MSC_VER
		...
		const std::string& SetValueDouble(double v5) { return SetFormattedValue<double>("%f", v5); }
		const std::string& SetValueFloat(float v5) { return SetFormattedValue<float>("%f", v5); }
		const std::string& SetValueInt(int v5) { return SetFormattedValue<int>("%i",v5); }
		const std::string& SetValueUInt(unsigned int v5) { return SetFormattedValue<unsigned int>("%u",v5); }
		const std::string& SetValueLongLong(long long v5) { return SetFormattedValue<long long >("%lli",v5); }
		const std::string& SetValueULongLong(unsigned long long v5) { return SetFormattedValue<long long >("%llu",v5); }

		XMLContent&  operator =(const char* s);
		XMLContent&  operator =(const std::string& s);
		const std::string& SetBinaryValue(const char* data,size_t len);

		void SetValue(const char* VV);
		void SetWideValue(const wchar_t* VV);
		void SetValue(const std::string& VV);
		void SetValue(const std::wstring& VV);
		void Clear();
		...
		[[nodiscard]] int GetValueInt(int def = 0)
			{
			return GetFormattedValue<int>("%i",def);
			}
		[[nodiscard]] unsigned int GetValueUInt(unsigned int def = 0)
			{
			return GetFormattedValue<unsigned int>("%u",def);
			}
		[[nodiscard]] long long GetValueLongLong(long long def = 0)
			{
			return GetFormattedValue<long long>("%lli",def);
			}
		[[nodiscard]] unsigned long long GetValueULongLong(unsigned long long def = 0)
			{
			return GetFormattedValue<unsigned long long>("%llu",def);
			}
		[[nodiscard]] float GetValueFloat(float def = 0)
		{
			return GetFormattedValue<float>("%f", def);
		}
		[[nodiscard]] double GetValueDouble(double def = 0)
		{
...
#ifdef _WIN32
		[[nodiscard]] std::wstring GetWideValue(int CP = CP_UTF8) const;
#endif
		[[nodiscard]] std::string GetValueDefault(const char* def) const;
		size_t GetEP() const;

		// Serialization
		virtual std::string Serialize(XMLSerialization* srz = 0) const;
...
class XMLVariable : public XMLContent
	...
	//	XMLVariable& operator =(const std::initializer_list<string>& s); 
		...
		const std::string& SetName(const char* VN);
		const std::string& SetName(const std::string& VN);
		void Clear();
		XMLVariable&  operator =(const char* s);
		XMLVariable&  operator =(const std::string& s);
...
class XMLElement
	...
	private:
		...
		std::vector<std::shared_ptr<XMLVariable>> variables;
		std::vector<std::shared_ptr<XMLContent>> contents;
		std::vector<std::shared_ptr<XMLComment>> comments;
		std::vector<std::shared_ptr<XMLCData>> cdatas;
	...
	public:
		...
		const std::vector<std::shared_ptr<XMLComment>>& GetComments() const { return comments; }
		const std::vector<std::shared_ptr<XMLElement>>& GetChildren()  const { return children; }
		const std::vector<std::shared_ptr<XMLVariable>>& GetVariables() const { return variables; }
		const std::vector<std::shared_ptr<XMLCData>>& GetCDatas()  const { return cdatas; }
		const std::vector<std::shared_ptr<XMLContent>>& GetContents()  const { return contents; }

		std::vector<std::shared_ptr<XMLComment>>& GetComments() { return comments; }
		std::vector<std::shared_ptr<XMLElement>>& GetChildren()  { return children; }
		std::vector<std::shared_ptr<XMLVariable>>& GetVariables() { return variables; }
		std::vector<std::shared_ptr<XMLCData>>& GetCDatas()  { return cdatas; }
		std::vector<std::shared_ptr<XMLContent>>& GetContents()  { return contents; }
		...
		const std::string& v(size_t idx) const;
		std::wstring wv(const char* nn);
		const std::string& v(const char* nn);
		std::string vd(const char*nn,const char*def = 0);
		std::string vd(const char*nn,const char*def = 0) const;
		...
		void GetAllChildren(std::vector<std::shared_ptr<XMLElement>>& ch) const;
//		shared_ptr<XMLElement> GetParent(shared_ptr<XMLElement> r) const;
		...
		// Find
		std::shared_ptr<XMLElement> FindElementZ(const char* n,bool ForceCreate = false);
		std::shared_ptr<XMLVariable> FindVariableZ(const char* n,bool ForceCreate = false,const char* defv = "",size_t* pidx = 0);
		std::shared_ptr<XMLVariable> FindVariable(const char* n) const;

		// Inserts
		std::shared_ptr<XMLElement> InsertElement(size_t y,const XMLElement& x);
		std::shared_ptr<XMLElement> InsertElement(size_t y,XMLElement&& x);
		XMLElement& AddElement(const XMLElement& c);
		XMLElement& AddElement(XMLElement&& c);
		XMLElement& AddElement(const char* n = "");
		std::shared_ptr<XMLElement> AddElement2(const char* n = "");
		void AddElements(const std::initializer_list<std::string>& s);
		void SetVariables(const std::initializer_list<std::string>& s);
		...
		std::shared_ptr<XMLElement> RemoveElementAndKeep(size_t i);
		...
		std::shared_ptr<XMLVariable> RemoveVariableAndKeep(size_t i);

		std::string EorE(const std::string& s,bool N) const;
		std::string Serialize(XMLSerialization* srz = 0) const;

		void Serialize(std::string& v, XMLSerialization* srz = 0) const;

		// Crypto
#ifdef _WIN32
		HRESULT Verify();
		HRESULT Sign(std::vector<PCCERT_CONTEXT> cert,const wchar_t* algo = L"SHA256",const LPCWSTR sl = L"/Signature");
		std::string Encrypt(const char* d, ULONG l);
#endif
...
class XML
	...
	private:
		...
		XMLHeader hdr;
		XMLDocType doctype;
		std::vector<std::shared_ptr<XMLComment>> hdrcomments;
		XMLElement root;
...
inline int _vscprintf(const char *format, va_list argptr)
	...
	inline std::string Char2Base64(const char* Z, size_t s, bool CRLF)
	{
		if (!s)
			return "";
#ifdef _WIN32
	...
	inline void BXML::ToB(std::string& s)
		{
	...
	inline std::string Decode(const char* src)
		{
		std::string trg;
		if (!src)
			return trg;
	...
	inline std::string Trim(const char* src,int m)
		{
		std::string s = src;
	...
	inline std::string Encode(const char* src)
		{
		std::string trg;
		if (!src)
			return trg;
	...
	inline XMLU::XMLU(const char* x)
		...
		std::unique_ptr<wchar_t> ws(new wchar_t[si]);
#ifdef _WIN32
	...
	inline XMLU::XMLU(const wchar_t* x)
		...
		std::unique_ptr<char> ws(new char[si]);
#ifdef _WIN32
	...
	inline XMLContent::operator const std::string& () const { return v; }

	// Sets
	inline const std::string& XMLContent::SetFormattedValue(const char* fmt,...)
		{
		va_list args;
		va_start(args,fmt);
	...
	inline XMLContent&  XMLContent::operator =(const std::string& s)
		{
		v = s;
		return *this;
		}

	inline const std::string& XMLContent::SetBinaryValue(const char* data,size_t len)
		{
		if (!len || !data)
			return v;
	...
	inline void XMLContent::SetValue(const std::string& VV)
		{
		v = VV;
		}
	inline void XMLContent::SetValue(const std::wstring& VV)
	{
		v = XMLU(VV.c_str());
	}
	...
	inline const std::string& XMLContent::GetValue() const
		{
		return v;
		}

	#ifdef _WIN32
	inline std::wstring XMLContent::GetWideValue(int CP) const
		{
	...
	inline std::string XMLContent::GetValueDefault(const char* s) const
		{
		if (v.length())
			return v;
		return s;
	...
	inline std::string XMLContent::trim(const std::string& str,
		const std::string& whitespace)
		{
	...
	// Serialization
	inline std::string XMLContent::Serialize(XMLSerialization* srz) const
		{
...
*/
	inline const std::string& XMLVariable::SetName(const char* VN)
		{
		if (!VN)
			return n;
		n = VN;
	...
	inline const std::string& XMLVariable::SetName(const std::string& VN)
		{
		n = VN;
		n.erase(n.find_last_not_of(" \n\r\t") + 1);
		return n;
	...
	inline XMLVariable&  XMLVariable::operator =(const std::string& s)
		{
		v = s;
		return *this;
		}
	...
	// Gets
	inline const std::string& XMLVariable::GetName() const
		{
		return n;
		}
	...
	inline std::string XMLVariable::GetNS() const
	{
		return "";//*
	}
	...
	// Serialization
		inline std::string XMLVariable::Serialize(XMLSerialization* srz) const
		{ 
		XMLSerialization defsrz;
		if (!srz)
			srz = &defsrz;
	...
	// Serialization
	inline std::string XMLCData::Serialize(XMLSerialization* srz) const
	{
		XMLSerialization defsrz;
		if (!srz)
			srz = &defsrz;
	...
	// Serialization
	inline std::string XMLDocType::Serialize(XMLSerialization* srz) const
	{
		XMLSerialization defsrz;
		if (!srz)
			srz = &defsrz;
	...
	// Serialization
	inline std::string XMLComment::Serialize(XMLSerialization* srz) const
	{
		XMLSerialization defsrz;
		if (!srz)
			srz = &defsrz;
	...
	// Serialization
	inline std::string XMLHeader::Serialize() const
		{
	...
	inline XMLElement::XMLElement(const XMLElement&from)
		...
		for (auto a : from.cdatas)
			{
			std::shared_ptr<XMLCData> c = std::make_shared<XMLCData>(XMLCData(a->ep,a->v.c_str()));
			cdatas.push_back(c);
			}
		...
		for (auto a : from.comments)
			{
			std::shared_ptr<XMLComment> c = std::make_shared<XMLComment>(XMLComment(a->ep,a->v.c_str()));
		...
		for (auto a : from.contents)
			{
			std::shared_ptr<XMLContent> c = std::make_shared<XMLContent>(XMLContent(a->ep,a->v.c_str()));
		...
		for (auto a : from.variables)
			{
			std::shared_ptr<XMLVariable> c = std::make_shared<XMLVariable>(XMLVariable(a->n.c_str(),a->v.c_str()));
		...
		for (auto a : from.children)
			{
			std::shared_ptr<XMLElement> c = std::make_shared<XMLElement>(XMLElement());
...
//			operator=(from.Serialize().c_str());
	...
	inline XMLElement& XMLElement::operator [](const char* elm)
		...
		std::shared_ptr<XMLElement> e = FindElementZ(elm,true);
		return *e;
		}

	inline std::string XMLElement :: vd(const char*nn,const char*def)
		{
		std::string k;
		if (!nn)
			{
			...
			{
			std::shared_ptr<XMLVariable> v = FindVariableZ(nn,true);
			k = v->GetValue();
			}
		if (k.empty() && def)
			k = def;
	...
	inline std::string XMLElement::vd(const char*nn,const char*def) const
		{
		std::string k;
		if (!nn)
			{
			...
			{
			std::shared_ptr<XMLVariable> v = FindVariable(nn);
			if (v)
				k = v->GetValue();
			}
		if (k.empty() && def)
	...
	inline const std::string& XMLElement::v(size_t idx) const
		{
	...
	inline const std::string& XMLElement::v(const char* nn)
		{
		if (!nn)
			{
			if (variables.size() == 0)
	...
	#ifdef _WIN32
	inline std::wstring XMLElement::wv(const char* nn)
		{
		if (!nn)
			{
			if (variables.size() == 0)
	...
	inline std::string XMLElement::Content() const
		{
		if (contents.empty())
			return "";
	...
	inline std::string XMLElement::GetContent() const
		{
		return Content();
		}
	...
	inline const std::string& XMLElement::GetElementName() const
		{
		return el;
		}

	inline void XMLElement::GetAllChildren(std::vector<std::shared_ptr<XMLElement>>& ch) const
		{
		for (auto& a : children)
			{
			ch.push_back(a);
...
*/
	...
	inline XMLElement* XMLElement::GetParent(XMLElement* r) const
		...
		if (r->id == parent)
			return r;
		std::vector<std::shared_ptr<XMLElement>> ch;
		r->GetAllChildren(ch);
		for (auto a : ch)
			{
			if (a->id == parent)
	...
	inline XML_ERROR XMLElement::MoveElement(size_t i,size_t y)
		...
		std::shared_ptr<XMLElement> x = children[i];
	...
	// Find
	inline std::shared_ptr<XMLElement> XMLElement::FindElementZ(const char* n,bool ForceCreate)
		{
		if (!n)
			return 0;
		for (size_t i = 0; i < children.size(); i++)
			{
			std::shared_ptr<XMLElement>& cc = children[i];
			const std::string& cn = cc->GetElementName();
	...
	inline std::shared_ptr<XMLVariable> XMLElement::FindVariable(const char* n) const
		{
		if (!n)
			return 0;
		for (size_t i = 0; i < variables.size(); i++)
			{
			const std::shared_ptr<XMLVariable>& cc = variables[i];
			const std::string& cn = cc->GetName();
			if (strcmp(cn.c_str(),n) == 0)
				return cc;
			}
		return 0;
	...
	inline std::shared_ptr<XMLVariable> XMLElement::FindVariableZ(const char* n,bool ForceCreate,const char* defv,size_t* pidx)
		{
		if (!n)
			return 0;
		for (size_t i = 0; i < variables.size(); i++)
			{
			std::shared_ptr<XMLVariable>& cc = variables[i];
			const std::string& cn = cc->GetName();
			if (strcmp(cn.c_str(), n) == 0)
			{
				if (pidx)
	...
	// Inserts
	inline std::shared_ptr<XMLElement> XMLElement::InsertElement(size_t y,const XMLElement& x)
		{
		if (y >= children.size())
			y = children.size();
		std::shared_ptr<XMLElement> xx = std::make_shared<XMLElement>(XMLElement(x));
		children.insert(children.begin() + y,xx);
		children[y]->parent = id;
		children[y]->par = this;
		return children[y];
		}
	inline std::shared_ptr<XMLElement> XMLElement::InsertElement(size_t y,XMLElement&& x)
		{
		if (y >= children.size())
			y = children.size();
		std::shared_ptr<XMLElement> xx = std::make_shared<XMLElement>(XMLElement(std::forward<XMLElement>(x)));
		children.insert(children.begin() + y,xx);
		children[y]->parent = id;
		children[y]->par = this;
		return children[y];
	...
	inline std::shared_ptr<XMLElement> XMLElement::AddElement2(const char* n)
	{
		XMLElement c = n;
		return InsertElement((size_t)-1, std::forward<XMLElement>(c));
	}

	inline void XMLElement::AddElements(const std::initializer_list<std::string>& s)
		{
		for (auto& a : s)
			{
	...
	inline void XMLElement::SetVariables(const std::initializer_list<std::string>& s)
		{
		if (s.size() % 2)
			return;
	...
	inline XMLVariable& XMLElement::AddVariable(const XMLVariable& vv,size_t p)
		...
		std::shared_ptr<XMLVariable> v = std::make_shared<XMLVariable>(XMLVariable(vv));
	...
	inline XMLVariable& XMLElement::AddVariable(const char* vn,const char* vv,size_t p)
		...
		if (p == (size_t)-1)
			p = (size_t)variables.size();
		std::shared_ptr<XMLVariable> v = std::make_shared<XMLVariable>(XMLVariable(vn,vv));
		variables.insert(variables.begin() + p,v);
		return *variables[p];
		}
	...
	inline XMLContent& XMLElement::AddContent(const XMLContent& co, size_t pos)
		{
		if (pos == (size_t)-1)
			pos = (size_t)contents.size();
		std::shared_ptr<XMLContent> v = std::make_shared<XMLContent>(co);
	...
	inline XMLContent& XMLElement::AddContent(const char* pv,size_t ep,size_t p)
		...
		std::shared_ptr<XMLContent> v = std::make_shared<XMLContent>(XMLContent(ep,pv));
	...
	inline XMLComment& XMLElement::AddComment(const char* pv,size_t ep,size_t p)
		...
		std::shared_ptr<XMLComment> v = std::make_shared<XMLComment>(XMLComment(ep,pv));
	...
	inline XMLCData& XMLElement::AddCData(const char* pv,size_t ep,size_t p)
		...
		std::shared_ptr<XMLCData> v = std::make_shared<XMLCData>(XMLCData(ep,pv));
	...
	inline std::shared_ptr<XMLElement> XMLElement::RemoveElementAndKeep(size_t i)
		{
		if (i >= children.size())
	...
	inline std::shared_ptr<XMLVariable> XMLElement::RemoveVariableAndKeep(size_t i)
		{
		if (i >= variables.size())
			throw XML_ERROR::INVALIDARG;
		auto v = variables[i];
	...
	inline std::string XMLElement::EorE(const std::string& s,bool N) const
		{
		if (N)
			return s;
	...
	inline std::string XMLElement::Serialize(XMLSerialization* srz) const
		{
		std::string v;
		Serialize(v,srz);
		return v;
	...
	inline void XMLElement::Serialize(std::string& v, XMLSerialization* srz) const
		{
		XMLSerialization defsrz;
		if (!srz)
			srz = &defsrz;
		...
		if (!srz->ExcludeSelf)
			...
			if (!variables.empty())
				{
				std::vector<std::shared_ptr<XMLVariable>> v2;
				if (srz->Canonical)
				{
					v2 = variables;
				...
				const std::vector<std::shared_ptr<XMLVariable>>& v3 = srz->Canonical ? v2 : variables;
		...
		auto ac = [&] (std::vector<std::shared_ptr<XMLContent>>& cx,size_t B,size_t B2)
			{
			for (auto& a : contents)
				{
		...
		for (size_t i = 0; i < children.size(); i++)
			...
			// Before items
			std::vector<std::shared_ptr<XMLContent>> cx;
			ac(cx,i,i);
			for (auto& n : cx)
				{
				std::string e = XMLContent::trim(n->Serialize(srz));
				if (srz->Canonical)
				{
					v += e;
					//v += "\n";
...
//			if (srz->NoContentIndent)
	...
	inline XML_ERROR XML::Save(const char* f,XMLSerialization* se) const
		...
		std::string s = Serialize(se);
		if (hdr.GetEncoding() == "UTF-16")
			{
			fwrite("\xFE\xFF",1,3,fp);
			std::wstring s2 = XMLU(s.c_str()).operator const wchar_t*();
...
//					XMLElement ne;
			...
			if (pe == PARSEELEMENT::ENDCOMMENT)
				...
				if (l)
					...
					hdrcomments.push_back(std::make_shared<XMLComment>(XMLComment(comm)));
				}
	...
	// ser
	inline std::string XML::Serialize(XMLSerialization* srz) const
		{
		XMLSerialization defsrz;
		if (!srz)
			srz = &defsrz;
